...
const AdminLayout = () => {
  ...
  const MORE_ITEMS = [
    { to: '/admin/parents',       key: 'parents',       label: 'Phụ huynh',      Icon: Icons.Parent },
    ...
    <div className="flex h-screen bg-slate-50 font-sans">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 h-full fixed left-0 top-0 z-50">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {/* Quản lý Phụ huynh: tạo tài khoản + liên kết học viên */}
          <Link to="/admin/parents" className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm ${isActive('parents') ? 'bg-primary/10 text-primary' : 'text-slate-600 hover:bg-slate-50'}`}>
            <Icons.Parent active={isActive('parents')} />
            <span className="flex-1">Phụ huynh</span>
