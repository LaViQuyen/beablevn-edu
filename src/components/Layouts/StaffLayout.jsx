...
const Icons = {
  // Icon Theo dõi lớp (bảng trình chiếu + biểu đồ)
  Tracking: ({ active }) => (
...
const StaffLayout = () => {
  ...
  const [apptPending, setApptPending] = useState(0); // badge lịch hẹn phụ huynh chờ xác nhận
  ...
  // Badge Lịch hẹn PH = số lịch hẹn 'pending' liên quan lớp phụ trách (admin: tất cả)
  useEffect(() => {
    if (!currentUser?.id) return;
    const isAdmin = currentUser.role === 'admin';
    const myClassIds = currentUser.assignedClasses || [];
  ...
  const TABS = [
    { key: 'tracking',      to: '/staff/tracking' },
    ...
    { key: 'appointments',  to: '/staff/appointments' },
  ...
  const MORE_ITEMS = [
    { to: '/staff/classes',       key: 'classes',       label: 'Học viên',     Icon: Icons.Class },
    { to: '/staff/notifications', key: 'notifications', label: 'Thông báo',    Icon: Icons.Noti },
    { to: '/staff/appointments',  key: 'appointments',  label: 'Lịch hẹn PH',  Icon: Icons.Calendar, badge: apptPending },
    ...
    <div className="flex h-screen bg-slate-50 font-sans">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 h-full fixed left-0 top-0 z-50">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {/* Theo dõi lớp: góc nhìn theo LỚP, trang chủ của Cổng Giáo vụ */}
          <Link to="/staff/tracking" className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm ${isActive('tracking') ? 'bg-primary/10 text-primary' : 'text-slate-600 hover:bg-slate-50'}`}>
            <Icons.Tracking active={isActive('tracking')} />
            <span className="flex-1">Theo dõi lớp</span>
          ...
          {/* Lịch hẹn phụ huynh với phòng Đào tạo */}
          <Link to="/staff/appointments" className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm ${isActive('appointments') ? 'bg-primary/10 text-primary' : 'text-slate-600 hover:bg-slate-50'}`}>
            <Icons.Calendar active={isActive('appointments')} />
            <span className="flex-1">Lịch hẹn PH</span>
            {apptPending > 0 ? (
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 h-16 flex items-center justify-around z-50 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <Link to="/staff/tracking" className={mobileLinkClass('tracking')}><Icons.Tracking active={isActive('tracking')} /><span className="text-[10px] font-medium">Lớp</span></Link>
