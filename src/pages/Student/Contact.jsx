    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-[#003366]">Liên hệ</h2>
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold border transition-all ${view === 'new' ? 'bg-slate-100 text-slate-600 border-slate-200' : 'bg-[#003366] text-white border-[#003366] hover:bg-[#002244]'}`}
        >
          {view === 'new' ? '← Quay lại' : (
      ...
      {view === 'new' && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 md:p-6 max-w-2xl">
          <h3 className="font-bold text-[#003366] mb-4">Gửi tin nhắn mới</h3>
          <form onSubmit={handleNewConversation} className="space-y-4">
            ...
            <button
              ...
              className="w-full bg-[#003366] text-white py-3 rounded-xl font-bold hover:bg-[#002244] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
        ...
        <div className="max-w-2xl space-y-2">
          {conversations.length === 0 ? (
            <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-10 text-center">
              ...
              <button onClick={() => setView('new')} className="mt-3 text-[#003366] text-sm font-bold hover:underline">Gửi tin nhắn đầu tiên →</button>
            </div>
          ) : conversations.map(conv => (
            <button
              key={conv.id}
              onClick={() => openConversation(conv)}
              className="w-full bg-white rounded-2xl border border-slate-100 shadow-sm p-4 text-left hover:shadow-md hover:border-blue-100 transition-all flex items-start gap-3"
            >
              {/* Avatar GV */}
                ...
                <div className="flex justify-between items-start mb-0.5">
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-2">
                    {conv.studentUnread > 0 && (
                      <span className="w-5 h-5 bg-[#003366] text-white text-[10px] font-bold rounded-full flex items-center justify-center">{conv.studentUnread}</span>
                    )}
      ...
      {view === 'inbox' && selectedConv && (
        <div className="max-w-2xl">
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-[#003366] mb-4 transition-colors font-medium"
          >
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="p-4 border-b border-slate-100 bg-slate-50">
              <p className="font-bold text-[#003366]">{selectedConv.subject}</p>
                ...
                return (
                  <div key={i} className={`flex ${isStudent ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${
                      ...
                      <p className={`text-[10px] mt-1 ${isStudent ? 'text-blue-200' : 'text-slate-400'}`}>
