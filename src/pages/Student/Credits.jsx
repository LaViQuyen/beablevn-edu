  ...
  return (
    <div className="space-y-6 animate-fade-in-up pb-10">
      ...
      {walletModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setWalletModal(false)}>
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-md w-full space-y-4 border border-slate-100" onClick={e => e.stopPropagation()}>
            <h3 className="font-bold text-primary text-base">Chọn phương thức thanh toán</h3>
      ...
      <div className="card-std p-5">
        <h3 className="text-sm font-bold text-primary mb-3 uppercase tracking-wide">Điểm Bonus theo lớp</h3>
      ...
      <div className="card-std p-5">
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          <h3 className="text-sm font-bold text-primary uppercase tracking-wide">🌿 Đổi quà Fresh Fit</h3>
        ...
        {cartCount > 0 && (
          <div className="mt-4 p-4 bg-primary-light border border-green-200 rounded-xl flex flex-col md:flex-row md:items-center gap-3">
            <div className="flex-1 min-w-0">
              ...
              <p className={`text-sm font-extrabold mt-1 ${cartTotal > Math.max(availableCredits, availablePlus) ? 'text-amber-700' : 'text-primary'}`}>
              ...
              <input
                type="text"
                className="w-full mt-2 p-2.5 border border-green-200 rounded-xl text-xs outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 bg-white transition-colors"
      ...
      {/* ===== LỊCH SỬ ĐỔI QUÀ ===== */}
      <div className="card-std p-5">
        <h3 className="text-sm font-bold text-primary mb-3 uppercase tracking-wide">Lịch sử đổi quà</h3>
                ...
                <div key={r.id} className="flex items-start justify-between gap-3 p-3.5 rounded-xl border border-slate-100 bg-slate-50/50">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <div className="flex items-center gap-1.5 flex-wrap justify-end">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase ${r.wallet === 'plus' ? 'bg-sky-50 text-sky-700 border-sky-200' : r.wallet === 'cash' ? 'bg-amber-50 text-amber-700 border-amber-300' : 'bg-primary-light text-primary border-green-200'}`}>
                    ...
                    {r.wallet === 'cash'
                      ...
                      : <span className="text-sm font-extrabold text-primary">−{r.totalCredits} ⭐</span>}
                  </div>
                </div>
              );
            })}
