...
const MyGrades = () => {
  ...
  const ScoreChart = ({ data }) => {
    ...
    const typeColor = { bonus: '#f59e0b', assignment: '#10b981', formative: '#3b82f6', summative: '#8b5cf6' };
        ...
        <div className="overflow-x-auto">
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full min-w-[300px]" style={{ height: H }}>
            ...
            <path
              ...
              fill="#003366" fillOpacity="0.06"
            />

            {/* Line */}
            <path d={linePath} fill="none" stroke="#003366" strokeWidth="2" strokeLinejoin="round" />

            {/* Points */}
            {data.map((d, i) => (
              <g key={i}>
                <circle cx={xScale(i)} cy={yScale(d.score)} r="4" fill={typeColor[d.type] || '#003366'} stroke="white" strokeWidth="1.5" />
        ...
        <div className="flex flex-wrap gap-3 mt-1">
          {Object.entries({ bonus: '⭐ Bonus', assignment: '📝 Assignment', formative: '📊 Formative', summative: '🎯 Summative' }).map(([k, v]) => (
            <span key={k} className="text-[10px] font-bold flex items-center gap-1 text-slate-500">
              <span className="w-2 h-2 rounded-full inline-block" style={{ background: { bonus: '#f59e0b', assignment: '#10b981', formative: '#3b82f6', summative: '#8b5cf6' }[k] }} />
              {v}
            </span>
          ))}
        </div>
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl border border-slate-200 shadow-sm">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-5 mb-6">
                <div className="w-full md:w-1/3">
                    ...
                    <select 
                        className="w-full p-2.5 border border-slate-200 rounded-xl outline-none focus:border-[#003366] focus:ring-2 focus:ring-[#003366]/10 font-medium bg-white"
                        value={selectedClass}
