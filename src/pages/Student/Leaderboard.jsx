...
const StudentLeaderboard = () => {
  ...
  const renderValue = (r) => {
    if (isExam) {
      if (r.id === currentUser?.id) return <span className="text-sm font-extrabold text-[#3D8B47]">{r.value.toFixed(1)} <span className="text-[10px] text-slate-400">điểm</span></span>;
    ...
    return <span className="text-sm font-extrabold text-[#3D8B47]">{r.value}{activeTab.unit ? ` ` : ''}<span className="text-[10px] text-slate-400 font-bold">{activeTab.unit}</span></span>;
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in-up pb-10">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`shrink-0 px-4 py-2.5 rounded-xl text-sm font-bold border-2 transition-colors ${tab === t.key ? 'border-[#2B6830] bg-[#E8F4EC] text-[#2B6830]' : 'border-slate-200 bg-white text-slate-500 hover:border-green-300'}`}>
            {t.icon} {t.label}
          </button>
        ))}
      </div>
      ...
      {/* MÔ TẢ HẠNG MỤC */}
      <div className="bg-[#E8F4EC]/50 border border-green-100 rounded-xl px-4 py-3">
        <p className="text-[13px] text-slate-600"><b className="text-[#2B6830]">{activeTab.icon} {activeTab.label}:</b> {activeTab.desc}</p>
      </div>
        ...
        <>
            ...
            {[1, 0, 2].map(pos => {
              ...
              return (
                <div key={r.id} className="flex flex-col items-center">
                  ...
                  <p className={`text-xs font-bold mt-1.5 text-center truncate w-full px-1 ${isMe ? 'text-[#2B6830]' : 'text-slate-700'}`}>{r.name}{isMe && ' (Bạn)'}</p>
                  <div className="text-[11px] font-extrabold">{renderValue(r)}</div>
                  <div className={`${heights[pos]} w-full mt-2 rounded-t-xl bg-gradient-to-t from-[#2B6830] to-[#3D8B47] flex items-start justify-center pt-2`}>
          ...
          <div className="card-std p-4">
            ...
            <div className="space-y-1.5">
              {top.map(r => (
                <div key={r.id} className={`flex items-center gap-3 p-2.5 rounded-xl border ${r.id === currentUser?.id ? 'bg-[#E8F4EC] border-[#2B6830]/30' : 'bg-white border-slate-100'}`}>
                  <span className={`w-7 text-center font-extrabold text-sm shrink-0 ${r.rank <= 3 ? 'text-[#2B6830]' : 'text-slate-400'}`}>
                    {r.rank <= 3 ? MEDAL[r.rank - 1] : r.rank}
                  </span>
                  <StudentAvatar skin={resolveSkin(r.id)} name={r.name} size={36} ring={false} />
                  <div className="flex-1 min-w-0">
                    <p className={`truncate text-sm font-bold ${r.id === currentUser?.id ? 'text-[#2B6830]' : 'text-slate-700'}`}>
                      {r.name}{r.id === currentUser?.id && <span className="text-[10px] font-bold text-[#2B6830] ml-1">• Bạn</span>}
                    </p>
            ...
            {me && !meInTop && (
              <>
                ...
                <div className="flex items-center gap-3 p-2.5 rounded-xl border bg-[#E8F4EC] border-[#2B6830]/30">
                  <span className="w-7 text-center font-extrabold text-sm shrink-0 text-[#2B6830]">{me.rank}</span>
                  <StudentAvatar skin={resolveSkin(me.id)} name={me.name} size={36} ring={false} />
                  <p className="flex-1 min-w-0 truncate text-sm font-bold text-[#2B6830]">{me.name} <span className="text-[10px] font-bold ml-1">• Bạn</span></p>
