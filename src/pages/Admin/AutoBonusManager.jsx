...
const AutoBonusManager = () => {
  ...
  const save = async () => {
    ...
    try {
      ...
      show('✅ Đã lưu cấu hình auto-bonus!');
    ...
    <div>
      ...
      <div className="flex items-center gap-2">
        <input type="number" min="0" step="0.5" value={rules[k]} onChange={e => setN(k, e.target.value)}
          className="w-full p-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-[#2B6830] focus:ring-2 focus:ring-[#2B6830]/10" />
  ...
  return (
    <div className="space-y-6 pb-20">
      {toast && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[80] px-5 py-3 rounded-xl shadow-lg text-sm font-medium text-white"
          style={{ background: toast.startsWith('✅') ? '#059669' : '#dc2626' }}>{toast}</div>
      )}
      ...
      {/* Bật/tắt tổng */}
      <label className={`flex items-center justify-between gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-colors ${rules.enabled ? 'border-[#2B6830] bg-[#E8F4EC]/50' : 'border-slate-200 bg-slate-50'}`}>
        <div>
        ...
        <input type="checkbox" checked={rules.enabled} onChange={e => setN('enabled', e.target.checked)} className="w-5 h-5 accent-[#2B6830]" />
      </label>
      ...
      <div className={rules.enabled ? '' : 'opacity-50 pointer-events-none'}>
        ...
        <div className="card-std p-5 mb-4">
          ...
          <label className="flex items-center gap-2 mt-4 cursor-pointer">
            <input type="checkbox" checked={rules.formativeCounts} onChange={e => setN('formativeCounts', e.target.checked)} className="w-4 h-4 accent-[#2B6830]" />
