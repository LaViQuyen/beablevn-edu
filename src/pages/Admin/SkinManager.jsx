...
const SkinManager = () => {
  ...
  // Toast phân loại: màu lấy TỪ type (success/error/warning), không suy từ emoji
  ...
  const seedDefaults = async () => {
    ...
    try {
      ...
      showToast('Đã nạp bộ skin mặc định!', 'success');
  ...
  const saveSkin = async () => {
    if (!form.name.trim()) return showToast('Nhập tên skin!', 'warning');
    setSaving(true);
    try {
      const payload = {
        name: form.name.trim(),
  ...
  const toggleStaff = async (skin) => {
    try {
      ...
      showToast(!skin.staffAllowed ? `Đã cấp "${skin.name}" cho nhân sự` : `Đã thu hồi "${skin.name}" khỏi nhân sự`, !skin.staffAllowed ? 'success' : 'warning');
    } catch (e) { showToast('Lỗi: ' + e.message, 'error'); }
  };
  ...
  const confirmDelete = async (id) => {
    try {
      ...
      showToast('Đã xóa skin. Học viên đang dùng skin này sẽ tự về Mầm Xanh.', 'success');
      setDeleteId(null);
    } catch (e) { showToast('Lỗi: ' + e.message, 'error'); }
  };
  ...
  return (
    <div className="space-y-6 pb-20">
      {/* TOAST, màu theo type */}
      {toast.msg && (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-[80] px-5 py-3 rounded-xl shadow-lg text-sm font-medium text-white max-w-[90vw] animate-fade-in-up ${toast.type === 'error' ? 'bg-red-500' : toast.type === 'warning' ? 'bg-amber-500' : 'bg-emerald-600'}`}>
          {toast.msg}
        </div>
      )}
            ...
            return (
              <div key={skin.id} className={`rounded-2xl border p-4 flex flex-col gap-3 transition-all ${skin.available ? 'border-slate-200 bg-white' : 'border-slate-200 bg-slate-50 opacity-70'}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                      ...
                      {skin.unlock === 'milestone'
                        ...
                        : <span className="text-[9px] font-bold px-1.5 py-0.5 rounded border uppercase bg-primary-light text-primary border-green-200">{skin.cost} credit</span>}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleAvailable(skin)}
                    className={`flex-1 py-2 rounded-lg text-xs font-bold border transition-colors ${skin.available ? 'bg-primary-light text-primary border-green-200 hover:bg-green-100' : 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200'}`}>
                ...
                <button onClick={() => toggleStaff(skin)}
                  className={`w-full py-1.5 rounded-lg text-[11px] font-bold border transition-colors ${skin.staffAllowed ? 'bg-primary-light text-primary border-green-200 hover:bg-green-100' : 'bg-slate-50 text-slate-400 border-slate-200 hover:bg-slate-100'}`}>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[70] p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-primary text-base">{editId ? 'Sửa skin' : 'Thêm skin mới'}</h3>
            ...
            <div>
              ...
              <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                className="w-full p-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/10" placeholder="VD: Ngôi Sao May Mắn" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={form.emoji} onChange={e => setForm({ ...form, emoji: e.target.value })} maxLength={4}
                  className="w-full p-2.5 border border-slate-200 rounded-xl text-lg text-center outline-none focus:border-primary" placeholder="✨" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Hạng</label>
                <select value={form.rarity} onChange={e => setForm({ ...form, rarity: e.target.value })}
                  className="w-full p-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-primary bg-white">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <button onClick={() => setForm({ ...form, unlock: 'purchase' })}
                  className={`flex-1 py-2.5 rounded-xl text-xs font-bold border-2 transition-colors ${form.unlock === 'purchase' ? 'border-primary bg-primary-light/50 text-primary' : 'border-slate-200 text-slate-500'}`}>
                  💰 Đổi bằng Credit
                </button>
            ...
            {form.unlock === 'purchase' ? (
              <div>
                ...
                <input type="number" min="0" value={form.cost} onChange={e => setForm({ ...form, cost: e.target.value })}
                  className="w-full p-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-primary" />
              </div>
            ) : (
              <div>
            ...
            <label className="flex items-center gap-2 cursor-pointer bg-primary-light/50 border border-green-100 rounded-xl px-3 py-2.5">
              <input type="checkbox" checked={form.staffAllowed} onChange={e => setForm({ ...form, staffAllowed: e.target.checked })} className="w-4 h-4 accent-primary" />
              <span className="text-sm font-bold text-primary">👤 Cấp cho nhân sự (staff/giáo viên) dùng</span>
            </label>
