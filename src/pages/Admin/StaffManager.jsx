...
const ChangePasswordModal = ({ staff, onConfirm, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full space-y-4 border border-slate-100">
        <h3 className="font-bold text-[#003366] text-base">Đổi mật khẩu: {staff.name}</h3>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1">Mật khẩu mới</label>
            <input
              type="text"
              className="w-full border border-slate-200 p-3 rounded-xl text-sm outline-none focus:border-[#003366] focus:ring-2 focus:ring-[#003366]/10 transition font-mono"
              value={newPass}
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
        ...
        <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input className="border border-slate-200 p-3 rounded-xl text-sm outline-none focus:border-[#003366] focus:ring-2 focus:ring-[#003366]/10 transition" placeholder="Họ tên" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
          <div className="flex gap-2">
            <input className="border border-slate-200 p-3 rounded-xl w-1/2 text-sm outline-none focus:border-[#003366] focus:ring-2 focus:ring-[#003366]/10 transition" placeholder="ID Đăng nhập" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} />
            <input className="border border-slate-200 p-3 rounded-xl w-1/2 text-sm outline-none focus:border-[#003366] focus:ring-2 focus:ring-[#003366]/10 transition font-mono" placeholder="Mật khẩu" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
          </div>
          <select className="border border-slate-200 p-3 rounded-xl text-sm outline-none focus:border-[#003366] focus:ring-2 focus:ring-[#003366]/10 bg-white transition" value={formData.subRole} onChange={e => setFormData({...formData, subRole: e.target.value})}>
          ...
          <div className="bg-slate-50 p-4 rounded-xl h-32 overflow-y-auto border border-slate-200">
            ...
            {availableClasses.map(c => (
              <label key={c.id} className="flex items-center gap-2 text-sm mb-1 cursor-pointer hover:text-[#003366] transition-colors">
                <input type="checkbox" className="accent-[#003366] w-4 h-4" checked={formData.assignedClasses.includes(c.id)} onChange={() => handleClassToggle(c.id)} />
                {c.name}
              </label>
            ))}
          </div>
          <button className="md:col-span-2 bg-[#003366] text-white py-3 rounded-xl font-bold shadow-sm hover:bg-[#002244] transition-all active:scale-[0.98]">Tạo Tài khoản</button>
        </form>
      </div>
      ...
      {editingStaff && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[90vh]">
            <h3 className="font-bold text-lg mb-4 text-[#003366]">Điều chỉnh lớp: {editingStaff.name}</h3>
            <div className="flex-1 overflow-y-auto space-y-2 mb-4 bg-slate-50 p-3 rounded-xl border border-slate-200">
              {availableClasses.map((cls) => (
                <label key={cls.id} className="flex items-center space-x-3 cursor-pointer text-sm p-2 hover:bg-slate-100 rounded-xl">
                  <input type="checkbox" className="accent-[#003366] w-5 h-5" checked={(editingStaff.assignedClasses || []).includes(cls.id)} onChange={() => handleClassToggle(cls.id)} />
            ...
            <div className="flex gap-2 justify-end pt-2 border-t border-slate-100">
              ...
              <button onClick={() => { update(ref(db, `users/${editingStaff.id}`), { assignedClasses: editingStaff.assignedClasses }); setEditingStaff(null); showSuccess('Đã cập nhật lớp phụ trách.'); }} className="px-4 py-2 bg-[#003366] text-white rounded-xl text-sm font-bold hover:bg-[#002244] transition-colors">Lưu</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-4 md:p-6 rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#003366" className="w-5 h-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 6.75h12M8.25 12h12m-12 5.25h12M3.75 6.75h.007v.008H3.75V6.75zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zM3.75 12h.007v.008H3.75V12zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm-.375 5.25h.007v.008H3.75v-.008zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
            </svg>
            <h2 className="text-lg font-bold text-[#003366]">Danh sách Nhân sự ({filteredStaffList.length})</h2>
          </div>
          <div className="flex flex-col md:flex-row gap-2">
            <select className="p-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-[#003366] focus:ring-2 focus:ring-[#003366]/10 bg-slate-50" value={filterRole} onChange={(e) => setFilterRole(e.target.value)}>
              <option value="all">Tất cả vai trò</option>
              <option value="teacher">Giáo viên</option>
              <option value="cco">CCO</option>
              <option value="cca">CCA</option>
            </select>
            <select className="p-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-[#003366] focus:ring-2 focus:ring-[#003366]/10 bg-slate-50 md:min-w-[150px]" value={filterClass} onChange={(e) => setFilterClass(e.target.value)}>
                ...
                <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setEditingStaff(s)} className="text-[#003366] border border-[#003366] text-[10px] font-bold px-2 py-0.5 rounded-xl hover:bg-[#003366] hover:text-white transition-all">Sửa</button>
                    </div>
                  </td>
            ...
            <div key={s.id} className="p-4 border border-slate-200 rounded-xl bg-white shadow-sm flex flex-col gap-3">
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-[#003366] text-sm">{s.name}</h4>
              ...
              <div className="text-xs bg-slate-50 p-2 rounded-xl border border-slate-100">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={() => setEditingStaff(s)} className="text-[#003366] border border-[#003366] text-[10px] font-bold px-2 py-0.5 rounded-xl hover:bg-[#003366] hover:text-white transition-all whitespace-nowrap">Sửa</button>
                </div>
              </div>
